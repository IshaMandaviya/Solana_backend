  ...
  try {
    ...
    if (Nft && collection) {
      try {
        ...
        return res
          .status(201)
          .json({ message: "Nft is listed ", data: collection });
      } catch (error) {
      ...
      return res.status(404).json({
        success: false,
        message:
          "NFT is not part of any collection or collection does not exits",
      });
    }
  } catch (error) {
...
export const FetchListedNftsOfCollection = async (req, res) => {
  ...
  if (listedNfts == []) {
    return res
      .status(200)
      .send("There are No Nfts listed from this collection at the moment ");
  }
...
export const FetchListedOwnedNFTsInCollection = async (req, res) => {
  ...
  if (NFTs == undefined) {
    return res
      .status(400)
      .json(
        `NFT doesn't exist in this collection ${collectionName} for this owner ${owner}`
      );
  }
  ...
  try {
    if (mintKey) {
      ...
      if (nft && nft.inSale == true) {
        ...
        const owners = await Collection.findOne({
          owners: buyer,
          name: collectionName,
        });
        const nfts = await NFTS.find({
        ...
        if (nfts.length <= 1) {
          collection.owners = await collection.owners.filter(
